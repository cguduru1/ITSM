...
async function getDashboardStats() {
  try {
    const [ticketAgg, activeAssets, pendingChanges] = await Promise.all([
      ...
      Ticket.aggregate([
        {
          $facet: {
            total: [{ $count: "count" }],
            open: [
              { $match: { status: { $in: ["New", "Open", "In Progress", "On Hold"] } } },
              { $count: "count" }
            ],
            closed: [
      ...
      // 2. Asset Count
      typeof AssetMaster !== "undefined"
        ? AssetMaster.countDocuments({ isDeleted: { $ne: true } })
        : 0,

      // 3. Changes Count
      typeof Change !== "undefined"
        ? Change.countDocuments({ status: { $in: ["Pending", "Open", "Requested"] } })
        : 0
    ]);

    // Debug raw aggregation
    // console.log("🔍 Raw ticketAgg:", JSON.stringify(ticketAgg, null, 2));

    // Extract safely
    const result = ticketAgg && ticketAgg[0] ? ticketAgg[0] : {};
    
    const statsPayload = {
      totalTickets: result.total?.[0]?.count || 0,
      openTickets: result.open?.[0]?.count || 0,
      closedTickets: result.closed?.[0]?.count || 0,
      breachedTickets: result.breached?.[0]?.count || 0,
      activeAssets: activeAssets || 0,
      pendingChanges: pendingChanges || 0,
      ciHealth: 92,   // static example metric
      kbSuccess: 85   // static example metric
    };

    console.log("🚀 Server Sending Stats Payload:", statsPayload);
    return statsPayload;
